...
func TestMissedDailyQuestPenaltyIsAuditableAndIdempotent(t *testing.T) {
	...
	now := time.Now().UTC()
	if removed, err := svc.store.RollOverDailyQuests(svc.userID, now, nil); err != nil || removed != 0 {
	...
	removed, err := svc.store.RollOverDailyQuests(svc.userID, future, nil)
	if err != nil {
	...
	again, err := svc.store.RollOverDailyQuests(svc.userID, future, nil)
	if err != nil || again != 0 {
...
func TestCompletedDailyQuestAvoidsPenaltyAndRollsOver(t *testing.T) {
	svc := newTestService(t)
	now := time.Now().UTC()
	if _, err := svc.store.RollOverDailyQuests(svc.userID, now, nil); err != nil {
	...
	removed, err := svc.store.RollOverDailyQuests(svc.userID, now.AddDate(0, 0, 1), nil)
	if err != nil {
...
func TestRestModeWaivesMissedDailyPenalty(t *testing.T) {
	...
	restSince := now
	removed, err := svc.store.RollOverDailyQuests(svc.userID, now.AddDate(0, 0, 1), &restSince)
	if err != nil {
...
func TestDailyPenaltyFirstRunDoesNotChargeExistingQuests(t *testing.T) {
	svc := newTestService(t)
	removed, err := svc.store.RollOverDailyQuests(svc.userID, time.Now().AddDate(0, 0, 30), nil)
	if err != nil {
