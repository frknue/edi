...
func (s *Store) CompleteTool(userID int64, toolKey, toolName string, data []byte, summary string, rewards map[string]int64) (models.ToolEntry, []models.XPEvent, []models.LevelUp, error) {
	names, err := s.AttributeNames(userID)
	if err != nil {
		return models.ToolEntry{}, nil, nil, err
	}
