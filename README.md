...
Add a compact stats block to every new shell — silent (and exit 0) when the
server is unreachable, so it never breaks shell startup. In `.zshrc`:
...
Requires `bin/edi-cli` on your `PATH` (or use the absolute path, e.g.
`/absolute/path/to/bin/edi-cli status`).
...
server/
  ...
  internal/telegram/     Telegram Bot API client (long-poll getUpdates + sendMessage)
